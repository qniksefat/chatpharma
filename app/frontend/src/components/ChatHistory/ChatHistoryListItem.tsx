...
interface ChatHistoryListItemCellProps {
  item?: ChatAppRequest 
  onSelect: (item: ChatAppRequest | null) => void
}
...
export const ChatHistoryListItemCell: React.FC<ChatHistoryListItemCellProps> = ({ item, onSelect }) => {
  ...
  const handleSaveEdit = async (e: any) => {
    ...
    if (!response.ok) {
      ...
      setEdit(false)
      appStateContext?.dispatch({ type: 'UPDATE_CHAT_TITLE', payload: { ...item, title: editTitle } as ChatAppRequest })
      setEditTitle('')
    }
  }
...
export const ChatHistoryListItemGroups: React.FC<ChatHistoryListItemGroupsProps> = ({ groupedChatHistory }) => {
  ...
  const [, setSelectedItem] = React.useState<ChatAppRequest | null>(null)
  const [offset, setOffset] = useState<number>(25)
  const [observerCounter, setObserverCounter] = useState(0)
  const [showSpinner, setShowSpinner] = useState(false)
  const firstRender = useRef(true)

  const handleSelectHistory = (item?: ChatAppRequest) => {
    if (item) {
      setSelectedItem(item)
    }
  }

  const onRenderCell = (item?: ChatAppRequest) => {
